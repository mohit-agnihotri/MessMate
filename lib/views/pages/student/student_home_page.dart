...
class StudentHomePage extends ConsumerWidget {
  ...
  Duration _cutoffForSlot(MessModel mess, String slot, bool isToday) {
    ...
    final cutoff = DateTime(mealDate.year, mealDate.month, mealDate.day, h, m)
          ...
          child: CustomScrollView(
            slivers: [
              ...
              if (state.activeLeave != null)
                SliverToBoxAdapter(child: _OnLeaveBanner(state: state, ref: ref)),

              // ── Current Meal ──
              SliverToBoxAdapter(
                child: Padding(
                  padding: const EdgeInsets.fromLTRB(16, 20, 16, 10),
                  child: Text('Current Meal',
              ...
              SliverToBoxAdapter(
                child: Builder(builder: (ctx) {
                  ...
                  final enabledSlots = allSlots.where((s) =>
                    state.mess?.mealTimings[s]?['enabled'] == 'true').toList();
                  ...
                  final todayRemaining = state.currentSlot == 'closed'
                      ? <String>[]
                      : enabledSlots.where((s) =>
                          allSlots.indexOf(s) > currentIdx).toList();

                  // Tomorrow's slots
                  final tomorrowSlots = enabledSlots;
                  
                  final todayIso = DateTime.now().toIso8601String().substring(0, 10);
                  final tomorrowIso = DateTime.now().add(const Duration(days: 1)).toIso8601String().substring(0, 10);

                  // Combine: today remaining then tomorrow
                  final items = [
                    for (final s in todayRemaining)
                      _UpcomingItem(slot: s, isToday: true,
                          menu: state.todayMenus[s],
                          label: '${s[0].toUpperCase()}${s.substring(1)}',
                          isSkipped: state.skippedUpcomingMeals.containsKey('${todayIso}_$s')),
                    for (final s in tomorrowSlots)
                      _UpcomingItem(slot: s, isToday: false,
                          menu: state.tomorrowMenus[s],
                          label: "Tomorrow's ${s[0].toUpperCase()}${s.substring(1)}",
                          isSkipped: state.skippedUpcomingMeals.containsKey('${tomorrowIso}_$s')),
                  ];
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return _UpcomingMealCard(
                              ...
                              onTap: () => _showMealBottomSheet(
                                context, ref, state, item.slot, item.menu,
  ...
  Widget _buildCurrentMealCard(
      BuildContext context, WidgetRef ref, StudentHomeState state) {
    if (state.isLoading) {
      return const Center(
          child: Padding(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Action buttons
                    if (!hasCutoff)
                      _RateMealButton(context: context, ref: ref, state: state, menu: menu)
                      ...
                      Row(children: [
                        Expanded(
                          flex: 3,
                          child: _SkipButton(
                              ref: ref, mealSlot: menu.mealSlot),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          flex: 2,
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            if (menu != null) ...[
              Text(menu.dishes.map((d) => d.name).join(', '),
                  style: GoogleFonts.inter(
                      fontSize: 14,
                      color: const Color(0xFF374151),
                      height: 1.5)),
            ] else ...[
              Text('Menu not set yet.',
                  style: GoogleFonts.inter(
              ...
              Row(children: [
                Expanded(
                  ...
                  child: isSkipped 
                    ? ElevatedButton(
                        onPressed: () {
                          final mealIso = mealDate.toIso8601String().substring(0, 10);
                          final recordId = state.skippedUpcomingMeals['${mealIso}_$slot'];
                          if (recordId != null) {
                ...
                Expanded(
                  ...
                  child: OutlinedButton(
                    onPressed: () {
                      Navigator.pop(ctx);
                      _showAddGuestDialog(context, ref, state, slot, mealDate);
                    },
  ...
  void _showAddGuestDialog(BuildContext context, WidgetRef ref, StudentHomeState state, String slot, DateTime mealDate) {
    int count = 1;
    showDialog(
      context: context,
      builder: (ctx) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          title: Text('Add Guests', style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('How many guests are joining you?', style: GoogleFonts.inter(color: const Color(0xFF4B5563))),
              const SizedBox(height: 20),
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  ...
                  const SizedBox(width: 20),
                  IconButton(
                    onPressed: count < 10 ? () => setState(() => count++) : null,
                    icon: const Icon(Icons.add_circle_outline, color: Color(0xFF22C55E), size: 32),
                  ),
                ],
              ),
            ],
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ref.read(studentHomeProvider.notifier).addGuest(slot, state.mess?.perMealRate ?? 0.0, date: mealDate, count: count);
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
...
class _UpcomingItem {
  ...
  const _UpcomingItem({required this.slot, required this.isToday, required this.menu, required this.label, required this.isSkipped});
}
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.inter(
                      ...
                      fontWeight: FontWeight.bold,
                      color:
                          isClosed ? const Color(0xFF6B7280) : Colors.white,
                      letterSpacing: 0.5),
                ),
                Text(
...
class _UpcomingMealCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final timings = state.mess?.mealTimings[slot];
    final timeRange =
        timings != null ? '${timings['start']} - ${timings['end']}' : '';
    final icon = _slotIcons[slot] ?? '🍽';
    final dishesStr = menu?.dishes.map((d) => d.name).join(', ') ??
        'Menu not set';
              ...
              Expanded(
                child: Text(slotName,
                    style: GoogleFonts.inter(
                        ...
                        color: isSkipped ? const Color(0xFF9CA3AF) : const Color(0xFF111827))),
              ),
              if (isSkipped)
                Container(
                  ...
                  child: Text('SKIPPED', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: const Color(0xFFEF4444))),
                )
              else
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text('Mess Closed for Today',
              style: GoogleFonts.inter(
                  fontWeight: FontWeight.bold,
                  fontSize: 16,
                  color: const Color(0xFF111827))),
          const SizedBox(height: 4),
          Text('All meals for today have concluded.',
              style: GoogleFonts.inter(
        ...
        children: [
          ...
          Text('Menu Not Set',
              style: GoogleFonts.inter(
                  ...
                  color: const Color(0xFF111827))),
          const SizedBox(height: 4),
          Text('Owner has not set the menu for ${state.currentSlot} yet.',
              style: GoogleFonts.inter(
                  color: const Color(0xFF6B7280), fontSize: 13),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('You are On Leave',
                    style: GoogleFonts.inter(
                        fontWeight: FontWeight.bold,
                        fontSize: 15,
                        color: const Color(0xFF166534))),
                Text('Meals are auto-cancelled.',
                    style: GoogleFonts.inter(
...
class _RateMealButton extends StatelessWidget {
  ...
  void _showRatingDialog(BuildContext context, WidgetRef ref,
      StudentHomeState state, MenuModel menu) {
